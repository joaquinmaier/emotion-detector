...
def start_detecting_faces_from_video(video_path):
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            roi_color = img[y-rectangle_extra_size:y+h+rectangle_extra_size, x-rectangle_extra_size:x+w+rectangle_extra_size]

            if roi_color.shape[0] > 0 and roi_color.shape[1] > 0:
                cv2.imshow('face', roi_color)
                # Save the cropped face
                cv2.imwrite('./cropped_faces/face{}.jpg'.format(face_id), roi_color)
            else:
...
def face_already_detected(
    ...
    if len(previous_detected_faces) == 0: return [False, False]
